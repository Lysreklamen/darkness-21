...
class NorskFlaggBitmapLargeMovement : BaseScript() {
    override suspend fun run() {
        super.run()

	val start_angular_time: Int = 20	// Use this to set the starting phase of the waveyness
	val offset_x: Int = -35			// Use this to place the vertical bar of the cross, number of sub-pixels the bitmap is scrolled to the left
	val wavey_x: Float = 3.0f		// Use this to define horizontal waveyness (number of pixels to stray left/right from initial position)
	val wavey_y: Float = 3.5f		// Use this to define vertical waveyness (number of pixels to stray up/down from initial position)
	...
	var plane: BitMap = BitMap(allBulbs, res_x*sub_res, res_y*sub_res, -2.8f, -1.0f, 10.0f, 3.64f)
	effect(plane)
